...
// First declaration of SESSION Variables
$_SESSION = array_fill_keys(array(
    "name", "age", "sex", "weight" , "height",
    "sitreach",  "rotation", "broadj-size", "broadj-steps",
    "plank-min", "plank-sec", "pull", "flexhang",
    "PU", "PUmod", "cooper", "recovrate"), "");
    

// Store the email and package selected on the landing page into SESSION
if(isset($_GET["test-submit"])){
    // Check if email already in DB (eg after reloading the page)
    ...
    $sql = "SELECT * FROM fittestresults WHERE Email LIKE '".$_GET['test-email']."'";
    $mysqli_result = mysqli_query($con, $sql);
    $resultcount = mysqli_num_rows($mysqli_result);
    
    if( $resultcount == 0){
        $_SESSION["email"] = $_GET['test-email'];
        $_SESSION["sel-package"] = $_GET['sel_package'];
    
        // Create random unique token for user to be able to come back and for identification
        $usr_token = strtoupper(bin2hex(openssl_random_pseudo_bytes(5)));
        $_SESSION["usr_token"] = $usr_token;
    
        // The Email and Token should be saved to the DB
        // Create new line in DB and save results
        $sql =  "INSERT INTO fittestresults (ID, Timestamp, Email, UsrToken)";
    	$sql .= " VALUES (NULL, NULL, '".$_SESSION["email"]."', '".$_SESSION["usr_token"]."')";
        mysqli_query($con, $sql);
    
        // Connect the SESSION with the
        $_SESSION["submittedresultID"] = mysqli_insert_id($con);
    }
    mysqli_close($con);
    
    // Send welcome mail to user, with re-login link
    mail_registration( "de", $_SESSION["email"] , $_SESSION["usr_token"]  );
    
    // Set a Session Variable to show a notification box after first registration that an email has been sent.
    $_SESSION["email-sent"] = true;
    
    // Directly use the generated token, so that there is no error when refreshing the page.
    header("Location: test-de.php?sft=".$_SESSION['usr_token']);    
}

if(isset($_GET["sft"])){
    // If the unique user token is already existing, the corresponding data should be retrieved 
    // and updated instead of creating a new DB entry.
    $_SESSION["usr_token"] = $_GET['sft'];
    ...
    $sql = "SELECT ID, Name, Age, Sex, Weight , Height, Sitreach,  Rotation, Broadj_Size, Broadj_Steps, Plank_Min, Plank_Sec, Pull, Flexhang,";
    $sql .= "PU, PUmod, Cooper, Recovrate FROM fittestresults WHERE UsrToken = '".$_SESSION["usr_token"]."'";
    if($result = mysqli_query($con, $sql)){
        while ($row = mysqli_fetch_row($result)){
            // Store DB Results in SESSION variable
            $_SESSION["submittedresultID"] = $row[0];
            $_SESSION["name"] = $row[1];
            $_SESSION["age"] = $row[2];
            $_SESSION["sex"] = $row[3];
            $_SESSION["weight"] = $row[4];
            $_SESSION["height"] = $row[5];
            $_SESSION["sitreach"] = $row[6];
            $_SESSION["rotation"] = $row[7];
            $_SESSION["broadj-size"] = $row[8];
            $_SESSION["broadj-steps"] = $row[9];
            $_SESSION["plank-min"] = $row[10];
            $_SESSION["plank-sec"] = $row[11];
            $_SESSION["pull"] = $row[12];
            $_SESSION["flexhang"] = $row[13];
            $_SESSION["PU"] = $row[14];
            $_SESSION["PUmod"] = $row[15];
            $_SESSION["cooper"] = $row[16];
            $_SESSION["recovrate"] = $row[17];
        }    
    }
...
<!-- Facebook Conversion Code for Conversion_FitTest -->
<script>(function() {
...
window._fbq.push(['track', '6025875040489', {'value':'0.00','currency':'EUR'}]);
</script>
<noscript><img height="1" width="1" alt="" style="display:none" src="https://www.facebook.com/tr?ev=6025875040489&amp;cd[value]=0.00&amp;cd[currency]=EUR&amp;noscript=1" /></noscript>
<!-- Facebook Conversion Code for Conversion_FitTest_Dieting -->
<script>(function() {
  var _fbq = window._fbq || (window._fbq = []);
  if (!_fbq.loaded) {
    var fbds = document.createElement('script');
...
window._fbq = window._fbq || [];
window._fbq.push(['track', '6025876687489', {'value':'0.00','currency':'EUR'}]);
</script>
<noscript><img height="1" width="1" alt="" style="display:none" src="https://www.facebook.com/tr?ev=6025876687489&amp;cd[value]=0.00&amp;cd[currency]=EUR&amp;noscript=1" /></noscript>
...
$(document).ready(function(){
    setTimeout(function(){
        $("#notification-positive").fadeIn();
        setTimeout(function(){
            $("#notification-positive").fadeOut();    
        }, 3500);    
    }, 1000);    
});
</script>
...
<div class="color-overlay"> 
    ...
    <div data-role="content" class="sf-content">
        <?php if(!isset($_SESSION["submittedresultID"])) : ?>
        <div class="ui-responsive IDerror">
             <div class="sf-content-bg">
                <div class="sf-content-block">
                    <div class="sf-header-block">   
                        <h3><img src="images/Icon_Sad.png" style="height: 80px; width: 80px; margin-right: 20px"/> Uuuups, hier ist etwas schief gelaufen. </h3>
